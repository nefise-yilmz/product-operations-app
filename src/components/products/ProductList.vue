      ...
      <div class="col-12 mb-5 card mt-5 shadow">
        <div class="card-body">
          ...
          <table class="table table-hover table-striped table-bordered" v-if="getProducts.length>0">
            ...
            <tbody>
              <tr :key="product" v-for="product in getProducts">
                <td class="align-middle text-center">
                  <span class="badge badge-info"> {{ product.key }} </span>
                </td>
                <td class="align-middle text-center">{{ product.title }}</td>
                <td class="align-middle text-center" :class="setByCount(product.count)">{{ product.count }}</td>
                <td style="width: 120px">{{ product.price | currencyFilter }}</td>
